<div class="container rsrc-container" role="main">
			...
			<div class="top-infobox text-right col-xs-6">
				<?php if ( class_exists( 'WooCommerce' ) && get_theme_mod( 'my-account-link', 1 ) == 1 ) { // Login Register  ?>
					<?php if ( is_user_logged_in() ) { ?>
						<a class="my-account-link" href="<?php echo esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ); ?>" title="<?php esc_attr_e( 'Tài khoản', 'online-store' );/*esc_attr_e( 'My Account', 'online-store' );*/ ?>"><?php esc_html_e( 'Tài khoản', 'online-store' );/*esc_html_e( 'My Account', 'online-store' );*/ ?></a>
					<?php } else { ?>
						<a class="my-account-link" href="<?php echo esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ); ?>" title="<?php esc_attr_e( 'Đăng nhập | Đăng ký', 'online-store' );/*esc_attr_e( 'Login / Register', 'online-store' );*/ ?>"><?php esc_html_e( 'Đăng nhập | Đăng ký', 'online-store' );/*esc_html_e( 'Login / Register', 'online-store' );*/ ?></a>
					<?php } ?> 
				<?php } ?>
	...
	<div class="header-section header-alt header-alt-2 row" >
		<?php // Site title/logo ?>
		<header id="site-header" class="col-sm-3 hidden-xs rsrc-header text-left" role="banner">
				...
				<div class="header-search-form col-md-9 col-xs-8">
					...
					<form role="search" method="get" action="<?php echo get_permalink( woocommerce_get_page_id( 'shop' ) ); ?>">
      				...
      				foreach ( $categories as $category ) {
						...
						<input class="input-search col-sm-9 col-xs-12" name="s" type="text" placeholder="<?php esc_attr_e( 'Tìm kiếm sản phẩm', 'online-store' ); ?>"/>
						<!--<button type="submit"><?php /*esc_html_e( 'Tìm kiếm', 'online-store' ); */?></button>-->
					</form>
				</div>
			</div>
		<?php endif; ?>
